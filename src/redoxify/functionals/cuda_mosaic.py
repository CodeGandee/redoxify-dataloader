...
def dali_mosaic_images(images: DALINode, boxes: DALINode, labels: DALINode, pad_val: DALINode,
                       sub_image_scale: DALINode, center_ratio_range: DALINode, probability: DALINode):
    func = torch_python_function(
        images, boxes, labels, pad_val, sub_image_scale, center_ratio_range, probability,
...
def _dali_mosaic_images(images_list: List[torch.Tensor], boxes_list: List[torch.Tensor], labels_list: List[torch.Tensor], pad_val_list: List[torch.Tensor],
                        sub_image_scale_list: List[torch.Tensor], center_ratio_range_list: List[torch.Tensor], probabilities_list: List[torch.Tensor]):
    """Mosaic augmentation.
    ...
    for i, img_i in enumerate(images_list):
        ...
        img_i = torch.nn.functional.interpolate(
            img_i.unsqueeze(0), size=(int(h_i * scale_ratio_i), int(w_i * scale_ratio_i)),
    ...
    for img_idx, cur_img in enumerate(images_list):
        ...
        center_x = int(random.uniform(center_ratio_range[0], center_ratio_range[1]) * img_scale_w)
        center_y = int(random.uniform(center_ratio_range[0], center_ratio_range[1]) * img_scale_h)
        center_position = (center_x, center_y)
        mosaic_bboxes = []
        mosaic_bboxes_labels = []
        mosaic_img = torch.full((int(img_scale_h * 2), int(img_scale_w * 2), 3), pad_val_list[0], dtype=cur_img.dtype, device=cur_img.device)
        ...
        for i, loc in enumerate(loc_strs):
            ...
            gt_bboxes_i *= torch.tensor([w_i, h_i, w_i, h_i], dtype=gt_bboxes_i.dtype, device=gt_bboxes_i.device)
            ...
            gt_bboxes_i[:, 0::2] = torch.clamp(gt_bboxes_i[:, 0::2], min=0, max=img_scale_w * 2)
            gt_bboxes_i[:, 1::2] = torch.clamp(gt_bboxes_i[:, 1::2], min=0, max=img_scale_h * 2)
            ...
            gt_bboxes_i /= torch.tensor([img_scale_w * 2, img_scale_h * 2, img_scale_w * 2, img_scale_h * 2], dtype=gt_bboxes_i.dtype, device=gt_bboxes_i.device)
...
def _mosaic_combine(
        loc: str, center_position_xy: Sequence[float],
        img_shape_wh: Sequence[int], img_scale: Sequence[int]) -> Tuple[Tuple[int], Tuple[int]]:
    ...
    if loc == 'top_left':
        ...
        x1, y1, x2, y2 = max(center_position_xy[0] - img_shape_wh[0], 0), \
                            center_position_xy[1], \
                            center_position_xy[0], \
                            min(img_scale[1] * 2, center_position_xy[1] +
                                img_shape_wh[1])
...
if __name__ == '__main__':
    ...
    boxes = [torch.tensor([[0.25,0.25,0.75,0.75]], device='cuda') for _ in range(img_num)]
    ...
    for i, mosaic in enumerate(mosaics):
        ...
        for j, box in enumerate(mosaic_boxes[i]):
            box = box.cpu().numpy()
            box *= np.array([mosaic.shape[1], mosaic.shape[0], mosaic.shape[1], mosaic.shape[0]])
            box = box.astype(np.int32)
            label = mosaic_labels[i][j].item()
            cv2.rectangle(mosaic, (box[0], box[1]), (box[2], box[3]), (0, 255, 0), 2)
            cv2.putText(mosaic, str(label), (box[0], box[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
