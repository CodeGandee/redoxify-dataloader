...
fill_values_type = Union[
    int, float, List[int], List[float], Tuple[int, int], Tuple[float, float], DALINode
]
...
@define(kw_only=True, eq=False)
class MixupConfig:
    mixup_lower_ratio: Union[int, float, DALINode] = field(
        validator=av.instance_of((int, float, DALINode))
    )
    mixup_upper_ratio: Union[int, float, DALINode] = field(
        validator=av.instance_of((int, float, DALINode))
    )
    fill_values: fill_values_type = field(
        default=(114, 114, 114), validator=av.instance_of((int, float, DALINode, List))
    )
    probability: Union[float, DALINode] = field(
        default=0.5, validator=av.instance_of((int, float, DALINode))
    )
...
class Mixup(BaseTransform):
    def __init__(self, mixup_config: MixupConfig, inout_map: MixupInputOutputMap):
        self.m_mixup_config = mixup_config
        self.m_inout_map = inout_map
        assert (
            len(self.m_inout_map.image_mixup_settings) > 0
        ), "image_mixup_settings must be provided"
    ...
    def do_transform(
        ...
        mixup_params = MixupParams(mixup_indices=[0, 0])
    ...
    def _do_transform(
        ...
        assert len(self.m_inout_map.image_mixup_settings) == len(
        ...
        for img_setting, boxes_setting, labels_setting in zip(
            self.m_inout_map.image_mixup_settings,
            self.m_inout_map.box_mixup_settings,
            self.m_inout_map.label_mixup_settings,
        ):
            ...
            ratio = fn.random.uniform(
                range=(config.mixup_lower_ratio, config.mixup_upper_ratio),
                device="gpu",
            ...
            for sub_key in image_datablock.get_keys():
                ...
                if fn.random.coin_flip(probability=config.probability):
                    mixup_image, mixup_boxes, mixup_labels = dali_mixup_images(
                        image_data.gpu(), boxes_data.gpu(), labels_data.gpu(), ratio.gpu()
                    )
                else:
                    mixup_image = image_data.gpu()
                    mixup_boxes = boxes_data.gpu()
                    mixup_labels = labels_data.gpu()
