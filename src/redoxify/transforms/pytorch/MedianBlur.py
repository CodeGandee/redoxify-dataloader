...
from redoxify.functionals.cuda_blur import dali_median_blur_image
...
class ImageMedianBlurSetting:
    image_key : DataKey = field()
    output_key : DataKey = field()

@define(kw_only=True, eq=False)
class MedianBlurInputOutputMap:
    image_blur_settings : List[ImageMedianBlurSetting] = field(factory=list)
...
@define(kw_only=True, eq=False)
class MedianBlurParams(TransformParams):
    aug : Union[bool, DALINode] = field(default=True)
    
class MedianBlur(BaseTransform):
    def __init__(self, blur_config : MedianBlurConfig, 
                 inout_map : MedianBlurInputOutputMap):
    ...
    def do_transform(self, input_data : TransformInput, 
        ...
        blur_params = MedianBlurParams(aug=aug)
